...
impl RawSeat {
    ...
    /// builds these; it is public for a host that resolves its own route.
    #[must_use]
    pub fn new(
        id: impl Into<String>,
    ...
    fn session(&self, tools: Vec<Box<dyn Tool>>) -> Result<OpenHumanSessionHost> {
    ...
    pub async fn turn(
        ...
        Box::pin(CoreContext::scope(Arc::clone(&self.context), async {
