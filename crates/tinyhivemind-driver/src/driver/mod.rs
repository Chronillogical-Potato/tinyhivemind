...
pub use brief::{
    Channel, ConversationView, ElsewhereView, EpisodeBrief, speaker, standing_contract,
};
#[cfg(test)]
...
/// One pending canonical id and the exact bound handle.
pub struct PendingAgent<'a, A> {
    /// Canonical hive id.
    pub hive_agent_id: &'a str,
    /// The host's bound handle for that seat.
    pub agent: &'a A,
}
...
/// A bounded driver over one validated hive.
#[derive(Debug)]
pub struct CompletionDriver<'a, A> {
    hive: &'a BoundHive<A>,
    round_width: usize,
    queue_depth: usize,
...
impl<'a, A: BoundAgent> CompletionDriver<'a, A> {
    ...
    pub fn new(hive: &'a BoundHive<A>, round_width: usize) -> Result<Self> {
        if round_width == 0 {
