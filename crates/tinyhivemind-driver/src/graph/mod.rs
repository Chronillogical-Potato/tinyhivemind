//! One owned desk graph and the handles bound to its seats.
...
impl HiveGraph {
    /// Build an owned graph. Structural validation occurs in [`BoundHive::new`].
    #[must_use]
...
///
/// The host implements it for whatever it runs seats with: an agent handle
/// on a runtime it owns, a raw session it builds per turn, a worker it
/// reaches over a socket, or a plain name in a benchmark. The driver stores
/// the handle and hands it back with a pending round; it never runs one,
/// which is what makes the bound type the host's to choose and what keeps
/// this crate from naming any harness. `tinyhivemind-openhuman` implements
/// it for `OpenHuman`'s two kinds of seat.
...
pub struct AgentBinding<A> {
    ...
    /// The host's handle for the seat.
    pub agent: A,
}
...
/// A validated graph plus the handles bound to its seats.
#[derive(Clone, Debug)]
pub struct BoundHive<A> {
    graph: HiveGraph,
    bindings: BTreeMap<String, AgentBinding<A>>,
}

impl<A: BoundAgent> BoundHive<A> {
    /// Validate and own one complete hive.
    ///
    /// # Errors
    ///
    ...
    /// Look up the handle bound to a canonical hive id.
    #[must_use]
